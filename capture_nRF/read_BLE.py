...
for start, end in zip(start_times, end_times):
    ...
    axes[0].axvline(x=start, color="green", linestyle="--", label=f"Start: {start:.2f} µs (Active {duration:.2f} µs)")
    axes[0].axvline(x=end, color="red", linestyle="--", label=f"End: {end:.2f} µs (Active {duration:.2f} µs)")
